...
object TestRunner {
  ...
  def runModelTests(): Unit = {
    ...
    try {
      ...
      assert(farm.name == "Fazenda Teste", "Nome da fazenda deve estar correto")
      assert(farm.totalArea == 1000.0, "Área total deve estar correta")
      assert(farm.agriculturalArea + farm.vegetationArea == farm.totalArea, "Soma das áreas deve ser igual ao total")
      println("OK: Modelo Farm funcionando")
